...
public class MediaEncryptor {
    ...
    public MediaEncryptor(byte[] key) {
        this.mediaKey = key;
    }
    ...
    public byte[] encryptMedia(byte[] mediaData) {
        try {
            ...
            SecretKeySpec keySpec = new SecretKeySpec(mediaKey, "AES");
            GCMParameterSpec spec = new GCMParameterSpec(TAG_BITS, iv);
            cipher.init(Cipher.ENCRYPT_MODE, keySpec, spec);

            byte[] encrypted = cipher.doFinal(mediaData);
    ...
    public byte[] decryptMedia(byte[] obfuscatedData) {
        try {
    ...
    public void rotateMediaKey(byte[] newKey) {
        java.util.Arrays.fill(mediaKey, (byte) 0);
        System.arraycopy(newKey, 0, mediaKey, 0, Math.min(mediaKey.length, newKey.length));
    }
}
